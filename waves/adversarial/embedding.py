...
import sys
sys.path.append(os.path.join(os.path.dirname(__file__), 'adversarial'))
...
def adv_emb_attack_custom(
    wm_img, encoder, strength,  device
):
    ...
    # Generate adversarial images
    transform = transforms.ToTensor()
    wm_img = transform(wm_img).unsqueeze(0)
    image = wm_img.to(device)

    # PGD attack
    image_adv = attack.forward(image)

    transform = transforms.ToPILImage()
    image_adv = transform(image_adv.squeeze(0))
