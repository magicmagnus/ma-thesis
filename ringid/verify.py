...
def main():
    ...
    pipe = InversableStableDiffusionPipeline.from_pretrained(
        ...
        scheduler=scheduler,
            ...
            cache_dir="/is/sg2/mkaut/.cache/huggingface/hub"
    ...
    # in the paper, the discrete values are not from -64 to 64, with 2 levels of quantization,
    # but the value is chosen to be the std dev of the initial latents.
    # args.ring_value_range = base_latents.std().item() ca 0.999
    # or maybe use the std dev of the latents in frequency domain (which is very close to 64)
    # args.ring_value_range = fft(base_latents).std().item() ca 63.46

    print("[Info] setting ring_value_range to ", args.ring_value_range)
    ...
    print("[Info] Total number of key-value combinations: ", len(key_value_combinations))
    print("[Info] some Key-value combinations: ", key_value_combinations[:5])
    ...
    pattern_idx = 682
    pattern = Fourier_watermark_pattern_list[pattern_idx]
    # visualize watermark pattern before quantization
    ...
    plt.savefig(os.path.join('BEF_watermark_pattern.png'))
    plt.close()
    ...
    # visualize watermark pattern after quantization
    fig, ax = plt.subplots(2, 4, figsize=(10, 5))
    for i in range(4):
        ax[0, i].imshow(Fourier_watermark_pattern_list[pattern_idx][0, i].real.cpu().numpy(), cmap='GnBu', vmin=-50, vmax=50)
        ax[1, i].imshow(Fourier_watermark_pattern_list[pattern_idx][0, i].imag.cpu().numpy(), cmap='GnBu', vmin=-50, vmax=50)
    ax[0, 0].set_title('Real')
    ax[1, 0].set_title('Imag')
    plt.savefig(os.path.join('AFT1_watermark_pattern.png'))
    plt.close()
    ...
    ax[0, 0].set_title('Real')
    ax[1, 0].set_title('Imag')
    plt.savefig(os.path.join('AFT2_watermark_pattern.png'))
    plt.close()
    # Use a single ring pattern for verification
    Fourier_watermark_pattern = Fourier_watermark_pattern_list[pattern_idx]  # [64, -64, 64, -64, 64...], select this ring pattern
