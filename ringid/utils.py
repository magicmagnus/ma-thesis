...
USE_ROUNDER_RING = True # default True
 
HETER_WATERMARK_CHANNEL = [0] # default [0]
RING_WATERMARK_CHANNEL = [3] # default [3]
...
def generate_Fourier_watermark_latents(device, radius, radius_cutoff, watermark_region_mask, watermark_channel, original_latents = None, watermark_pattern = None):
    ...
    init_latents_orig = original_latents.clone()
    ...
    for i in range(4):
        ax[0, i].axis('off')
        ax[0, i].imshow(watermarked_latents_fft[0, i].real.cpu().numpy(), cmap='GnBu', vmin=-50, vmax=50)
    ax[0, 0].set_title('Original FFT of init_latents (real part)', loc='left', fontsize=10)

    # plot just the watermark_region_mask
    print('watermark_region_mask.shape', watermark_region_mask.shape) # torch.Size([2, 64, 64])
    # so in this case, watermark_region_mask is a list of two masks, one for each channel to be watermarked

    fig1, ax1 = plt.subplots(2, 1, figsize=(10, 10))
    for i in range(2):
        ax1[i].imshow(watermark_region_mask[i].cpu().numpy(), cmap='Greys', alpha=0.5)
    plt.tight_layout()
    plt.savefig('rid-orig-watermark-region-mask.png', bbox_inches='tight', pad_inches=0.2)
    plt.close(fig1)

    # plot only th e watermark pattern
    print("watermark_pattern.shape", watermark_pattern.shape) # torch.Size([1, 4, 64, 64])
    # watermark_pattern is on all channels, but only the ones specified in watermark_channel are copies into the final latents


    fig2, ax2 = plt.subplots(2, 4, figsize=(10, 10))
    for i in range(4):
        ax2[0, i].imshow(watermark_pattern[0, i].real.cpu().numpy(), cmap='GnBu', vmin=-50, vmax=50)
    ax2[0, 0].set_title('Watermark pattern (real part)', loc='left', fontsize=10)
    for i in range(4):
        ax2[1, i].imshow(watermark_pattern[0, i].imag.cpu().numpy(), cmap='GnBu', vmin=-50, vmax=50)
    ax2[1, 0].set_title('Watermark pattern (imag part)', loc='left', fontsize=10)
    plt.tight_layout()
    plt.savefig('rid-orig-watermark-pattern.png', bbox_inches='tight', pad_inches=0.2)
    plt.close(fig2)
    ### so the pattern has the ring-pattern only in the real part of the previously defined channel (RING_WATERMARK_CHANNEL)
    ### and the noise-pattern in the real AND imaginary part of the previously defined channel (HETER_WATERMARK_CHANNEL)
    ...
    for i in range(4):
        ax[1, i].axis('off')
        ax[1, i].imshow(watermarked_latents_fft[0, i].real.cpu().numpy(), cmap='GnBu', vmin=-50, vmax=50)
    ax[1, 0].set_title('FFT after watermarking (real part)', loc='left', fontsize=10)
    # and the imaginary part
    for i in range(4):
        ax[2, i].axis('off')
        ax[2, i].imshow(watermarked_latents_fft[0, i].imag.cpu().numpy(), cmap='GnBu', vmin=-50, vmax=50)
    ax[2, 0].set_title('FFT after watermarking (imag part)', loc='left', fontsize=10)

    watermarked_latents = torch.fft.ifft2(torch.fft.ifftshift(watermarked_latents_fft, dim = (-1, -2))).real

    for i in range(4):
        ax[3, i].axis('off')
        ax[3, i].imshow(watermarked_latents[0, i].cpu().numpy(), cmap='OrRd', vmin=-4, vmax=4)
    ax[3, 0].set_title('init_latents after watermarking (spatial domain)', loc='left', fontsize=10)

    diff = watermarked_latents - init_latents_orig
    for i in range(4):
        ax[4, i].axis('off')
        ax[4, i].imshow(diff[0, i].cpu().numpy(), cmap='RdBu', vmin=-4, vmax=4)
    ax[4, 0].set_title('Difference', loc='left', fontsize=10)

    plt.tight_layout()
    # Save the figure if needed
    plt.savefig('rid-orig.png', bbox_inches='tight', pad_inches=0.2)
    plt.close(fig)
...
def make_Fourier_ringid_pattern(
    ...
    #     ax[0, i].axis('off')
    #     ax[0, i].imshow(watermarked_latents_fft[0, i].real.cpu().numpy(), cmap='GnBu', vmin=-50, vmax=50)
    # ax[0, 0].set_title('FFT after "put ring" (real part)', loc='left', fontsize=10)
    # for i in range(4):
    #     ax[1, i].axis('off')
    #     ax[1, i].imshow(watermarked_latents_fft[0, i].imag.cpu().numpy(), cmap='GnBu', vmin=-50, vmax=50)
    # ax[1, 0].set_title('FFT after "put ring" (imag part)', loc='left', fontsize=10)
    ...
    # for i in range(4):
    #     ax[2, i].axis('off')
    #     ax[2, i].imshow(watermarked_latents_fft[0, i].real.cpu().numpy(), cmap='GnBu', vmin=-50, vmax=50)
    # ax[2, 0].set_title('FFT after "put noise or zeros" (real part)', loc='left', fontsize=10)
    # for i in range(4):
    #     ax[3, i].axis('off')
    #     ax[3, i].imshow(watermarked_latents_fft[0, i].imag.cpu().numpy(), cmap='GnBu', vmin=-50, vmax=50)
    # ax[3, 0].set_title('FFT after "put noise or zeros" (imag part)', loc='left', fontsize=10)

    # plt.tight_layout()
    # # Save the figure if needed
    # plt.savefig('rid-orig-only-pattern.png', bbox_inches='tight', pad_inches=0.2)
    # plt.close(fig)
