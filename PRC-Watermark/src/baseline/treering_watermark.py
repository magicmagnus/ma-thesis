...
def tr_get_noise(shape: Union[torch.Size, List, Tuple], keys_path, from_file: str = None, generator=None) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    if not from_file:
        ...
        for i in range(4):
            ax[0, i].imshow(init_latents[0, i].cpu().numpy(), cmap='Blues', vmin=-4, vmax=4)
        # set a colobar for the first row
        for i in range(4):
            im = ax[0, i].imshow(init_latents[0, i].cpu().numpy(), cmap='Blues', vmin=-4, vmax=4)
            if i == 0:
        ...
        for i in range(4):
            im = ax[1, i].imshow(np.abs(w_key[0, i].cpu().numpy()), cmap='Reds')
            if i == 0:
                fig.colorbar(im, ax=ax[1, i], orientation='vertical', label='w_key', location='left')
        ax[1, 0].set_title('w_key, frequency domain')
        ...
        for i in range(4):
            im = ax[2, i].imshow(np.abs(init_latents_fft[0, i].cpu().numpy()), cmap='Reds')
            if i == 0:
                fig.colorbar(im, ax=ax[2, i], orientation='vertical', label='init_latents_fft', location='left')

        ax[2, 0].set_title('init_latents_fft after watermarking, frequency domain')
        # plot the init_latents after watermarking
        for i in range(4):
            im = ax[3, i].imshow(init_latents[0, i].cpu().numpy(), cmap='Blues', vmin=-4, vmax=4)
            if i == 0:
